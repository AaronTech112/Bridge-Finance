...
function initChartAnimations() {
    ...
    statItems.forEach(item => {
        ...
        gsap.to(item, {
            ...
            onUpdate: function() {
                if (suffix.includes('%')) {
                    item.textContent = this.targets()[0].textContent.toFixed(1) + suffix;
                } else if (suffix.includes('M')) {
                    item.textContent = '$' + this.targets()[0].textContent.toFixed(1) + suffix;
                } else if (suffix.includes('+')) {
                    item.textContent = this.targets()[0].textContent.toFixed(0) + suffix;
                } else {
                    item.textContent = this.targets()[0].textContent.toFixed(0);
                }
            }
        });
    });
